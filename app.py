...
def envoi():
    ...
    liste_mot_utile=[]
    stop_words = set(stopwords.words('english')+["-",",",":", "''","&","@"]+stopwords.words('french')) #definiton des mots a retirer
    for key,value in (inputted_json.items()):#parcours du dictionnaire
        if (type(value) is dict):#et du dictionnaires de dictionnaires (boucle double for obligatoire)
            for key,value in value.items():
                if (value):
                    word_tokens = word_tokenize(unidecode.unidecode(value.lower()))#recuperation des donnees du dictionnaire ainsi que la position de la cle
